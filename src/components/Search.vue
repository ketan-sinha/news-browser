<template>
	<div class="w-full md:mx-6">
		<form v-on:submit.prevent="submitSearch" action=".">
			<input v-model.lazy="options.q" class="w-full appearance-none border-2 inline border-gray-400 focus:border-black max-w-sm md:max-w-3xl xl:max-w-6xl h-12 rounded-full px-4" name="search" type="search" placeholder="Search articles..."/>
		</form>
		<div class="mt-2 my-auto">
			<button @click="filterHidden = !filterHidden" title="Filters">
				<svg v-if="filterHidden" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	<transition name="slide-fade">
		<div v-if="!filterHidden" class="w-full text-center md:mx-6 mb-2 md:mb-3 z-0">
			<div class="block md:inline md:ml-6">
				<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" viewBox="0 0 20 20" fill="currentColor">
					<path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM4.332 8.027a6.012 6.012 0 011.912-2.706C6.512 5.73 6.974 6 7.5 6A1.5 1.5 0 019 7.5V8a2 2 0 004 0 2 2 0 011.523-1.943A5.977 5.977 0 0116 10c0 .34-.028.675-.083 1H15a2 2 0 00-2 2v2.197A5.973 5.973 0 0110 16v-2a2 2 0 00-2-2 2 2 0 01-2-2 2 2 0 00-1.668-1.973z" clip-rule="evenodd" />
				</svg>
				<select class="inline w-full max-w-xs md:max-w-max md:w-auto text-sm p-2 ml-1 md:ml-2 rounded-full" v-model="options.lang">
			...
			<div class="block md:inline mt-2 md:mt-0 md:ml-6">
				...
				<select class="inline w-full max-w-xs md:max-w-max md:w-auto text-sm p-2 ml-1 md:ml-2 rounded-full" v-model="options.topic">
			...
			<div class="block md:inline mt-2 md:mt-0 md:ml-6">
				<div class="block md:inline">
					...
					<input v-model="options.from" class="inline w-full max-w-xs md:max-w-max md:w-auto text-sm p-2 ml-1 md:ml-2 rounded-full" type="date">
				</div>
				<div class="mt-2 md:mt-0 md:ml-1 md:inline">
					...
					<div class="inline md:ml-1">
						<input v-model="options.to" class="inline w-full max-w-xs md:max-w-max md:w-auto text-sm p-2 ml-1 rounded-full" type="date">
					</div>
				</div>
			</div>
			<div class="inline-block relative align-middle ml-6 mt-2 md:mt-0 w-20">
